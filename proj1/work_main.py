...
# count of not present students
def count_notpresent3():
	i=0
	cnt11=0
	cnt22=0
	while i < 6:
...
def email_func(sender, receiver, filename, path, pos, neg):
	...
	s.login(sender, "abc_PASSWORD")          # PLEASE ENTER YOUR IITP EMAIL PASSWORD INSTEAD OF abc_PASSWORD
		...
		for roll in stud_info.keys():
			...
			if roll != "ANSWER":
				...
				if os.path.isfile(filename):
					...
					# PLEASE ENTER YOUR IITP EMAIL ID INSTEAD OF shubham_1901ee58@iitp.ac.in
					email_func("shubham_1901ee58@iitp.ac.in", stud_info[roll]["Email address"], roll+ ".xlsx", filename, pos, neg)
					email_func("shubham_1901ee58@iitp.ac.in", stud_info[roll]["IITP webmail"], roll+ ".xlsx", filename,  pos, neg)
